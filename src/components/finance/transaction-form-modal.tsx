...
import { X, Loader2, PlusCircle, ArrowDownLeft, ArrowUpRight } from "lucide-react";
import { createTransactionAction } from "@/app/(dashboard)/finance/actions";
...
export function TransactionFormModal({
  ...
  const [type, setType] = React.useState<"Pengeluaran" | "Pemasukan">(initialType);
  const [amount, setAmount] = React.useState<string>("");
  const [activity, setActivity] = React.useState<string>("");
  const [account, setAccount] = React.useState<string>(availableAccounts[0] || "Cash / Tunai");
  const [category, setCategory] = React.useState<string>(
    initialType === "Pemasukan" ? "Pemasukan / Gaji" : "Jajan / Makan"
  );
  ...
  React.useEffect(() => {
    if (isOpen) {
      setType(initialType);
      setCategory(initialType === "Pemasukan" ? "Pemasukan / Gaji" : "Jajan / Makan");
    }
  }, [isOpen, initialType]);

  // Sync account default if options change
  React.useEffect(() => {
    if (availableAccounts.length > 0 && !availableAccounts.includes(account)) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border/80 bg-surface-muted/30">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-text-primary">Catat Transaksi Baru</h2>
          </div>
          <button
            onClick={onClose}
            disabled={loading}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 p-1 bg-surface-muted rounded-lg border border-border">
              <button
                ...
                onClick={() => {
                  setType("Pengeluaran");
                  if (category === "Pemasukan / Gaji") setCategory("Jajan / Makan");
                }}
              ...
              <button
                ...
                onClick={() => {
                  setType("Pemasukan");
                  if (category === "Jajan / Makan") setCategory("Pemasukan / Gaji");
                }}
                className={`flex items-center justify-center gap-1.5 py-2 px-3 rounded-md text-xs font-semibold transition-all ${
                  type === "Pemasukan"
          ...
          <div>
            ...
            <input
              ...
              placeholder="Contoh: Beli lemineral 6900 cash"
